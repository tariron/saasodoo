#!/usr/bin/env python3
"""
User Service Direct Capacity Test
Tests user registration via user-service API (user-service -> billing-service -> KillBill).
"""
...
class UserServiceTester:
    def __init__(self, host: str):
        ...
        self.register_url = f"{self.host}/auth/register"
    ...
    def generate_email(self) -> str:
        ...
        return f"user_test_{ts}_{suffix}@loadtest.com"
    ...
    def run_test(self, total: int, concurrency: int) -> dict:
        ...
        if not self.check_health():
            return {"error": "User service health check failed"}

        print(f"Starting: {total} users, {concurrency} concurrent")
        print("-" * 50)
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as executor:
            futures = [executor.submit(self.register_user) for _ in range(total)]
...
def print_results(m: dict, title: str):
    ...
    print(f"{title}")
    print("=" * 60)
    print(f"\n  USERS PER MINUTE:          {m['per_minute']:>10.1f}")
    print(f"  USERS PER SECOND:          {m['per_second']:>10.2f}")
    print("-" * 60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="User service direct capacity test")
    parser.add_argument("--host", default="http://api.109.199.108.243.nip.io/user")
...
""")
    ...
    tester = UserServiceTester(args.host)
    metrics = tester.run_test(args.count, args.concurrent)
    print_results(metrics, "USER SERVICE DIRECT TEST RESULTS")

#!/usr/bin/env python3
"""
KillBill Direct Capacity Test
Tests account creation directly against KillBill API.
"""
...
class KillBillTester:
    def __init__(self, host: str):
        self.host = host.rstrip("/")
        self.account_url = f"{self.host}/1.0/kb/accounts"
        self.health_url = f"{self.host}/1.0/healthcheck"

        # KillBill auth
        self.auth = base64.b64encode(b"admin:password").decode()
        self.headers = {
            "Authorization": f"Basic {self.auth}",
            "X-Killbill-ApiKey": "fresh-tenant",
            "X-Killbill-ApiSecret": "fresh-secret",
            "X-Killbill-CreatedBy": "load-test",
            "Content-Type": "application/json"
        }

    def generate_external_key(self) -> str:
        ts = int(time.time() * 1000000)
        suffix = ''.join(random.choices(string.ascii_lowercase, k=4))
        return f"kb_test_{ts}_{suffix}"

    def generate_payload(self) -> dict:
        ext_key = self.generate_external_key()
        return {
            "externalKey": ext_key,
            "name": f"Test User {ext_key[:10]}",
            "email": f"{ext_key}@loadtest.com",
            "currency": "USD"
        }

    def check_health(self) -> bool:
        try:
            resp = requests.get(self.health_url, headers=self.headers, timeout=10)
    ...
    def create_account(self) -> TestResult:
        ...
        try:
            resp = requests.post(self.account_url, json=payload, headers=self.headers, timeout=30)
    ...
    def run_test(self, total: int, concurrency: int) -> dict:
        print("Checking KillBill health...")
        if not self.check_health():
            return {"error": "KillBill health check failed"}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="KillBill direct capacity test")
    parser.add_argument("--host", default="http://killbill.109.199.108.243.nip.io")
...
""")
    ...
    tester = KillBillTester(args.host)
    metrics = tester.run_test(args.count, args.concurrent)
    print_results(metrics, "KILLBILL DIRECT TEST RESULTS")
